...
public class UserController : Controller
{
    ...
    public async Task<IActionResult> FollowUnFollow(int? id)
    {
        ...
        {
            return BadRequest("Пользователь не может подписаться сам на себя");
        }
        ...
        return PartialView("_FollowButtonPartial", new ProfileViewModel
        {
            User = user,
            CurrentUser = currentUser,
            Subscriptions = await _context.Subscriptions
                .Where(s => s.SubscriberId == currentUser.Id).ToListAsync(),
        });
    }
